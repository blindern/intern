...
- **Backend** (`backend/`): Laravel, PHP, PostgreSQL. REST API at `/intern/api/`. Custom domain code under `app/src/` (PSR-4: `Blindern\Intern\`), standard Laravel code under `app/`. Auth via SAML2. All models use ULID primary keys (`HasUlids` trait). Array/nested fields use JSONB columns with Eloquent `$casts`.
